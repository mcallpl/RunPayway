...
const CONSTRAINT_TEXTS: Record<CanonicalKey, string> = {
  income_continuity_without_active_labor:
    "Income stops when active work stops. This creates a direct dependency between personal effort and revenue — meaning illness, time off, or capacity limits directly reduce income. This is the single largest structural vulnerability in this income system because it makes the entire revenue stream dependent on one person's sustained output.",
  ...
  income_concentration:
    "Revenue depends heavily on a small number of sources, creating outsized exposure to any single client or contract. If the largest source were to end, the impact on total income would be severe. This concentration risk is the structural equivalent of building on a single foundation pillar.",
  number_of_income_sources:
    "The income system draws from too few qualifying sources. With limited diversification, the loss of any single source represents a significant percentage of total income. Structural breadth — multiple independent sources each contributing meaningful revenue — is a core indicator of income durability.",
  earnings_variability:
    "Month-to-month income fluctuates significantly, creating unpredictable cash flow patterns. High variability makes financial planning difficult, increases stress, and can mask underlying structural weaknesses. Consistency — not just total earnings — is a key driver of long-term income stability.",
};
...
const DRIVER_TEXTS: Record<CanonicalKey, string> = {
  ...
  number_of_income_sources:
    "Multiple independent income sources contribute to total revenue, creating structural breadth. Each additional qualifying source reduces the proportional impact of any single loss.",
  forward_revenue_visibility:
