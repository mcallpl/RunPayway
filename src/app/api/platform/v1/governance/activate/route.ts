/**
 * POST /api/platform/v1/governance/activate
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await governanceService.activate(policy_id, version, context);
    ...
    if (error instanceof Error && error.message.includes("Cannot activate version")) {
      return NextResponse.json(
        {
          status: "error",
          error: "ConflictError",
