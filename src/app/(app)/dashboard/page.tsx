    ...
    <>
      ...
      <div style={{ minHeight: "100vh", backgroundColor: B.bg, fontFamily: sans }}>
        ...
        <div style={{ maxWidth: 960, margin: "0 auto", padding: mobile ? "20px 16px 120px" : "48px 36px 96px", overflow: "hidden" }}>
          ...
          {/* ── THIS WEEK — the first thing they see ── */}
          {topMoves.length > 0 && (() => {
            ...
            const projectedWithProgress = qCount > 0 ? qResult.overall_score : dScore;
            const progressLift = qCount > 0 ? qLift : 0;

            const copyBriefing = () => {
              const text = `RunPayway™ Weekly Briefing — ${custName || "Assessment"}\n\nScore: ${dScore}/100 (${dBand})\n${gap > 0 ? `${gap} points to ${nextB} Stability` : "Highest band achieved"}\n\nThis week's priority: ${nextMove?.label || "Complete Step 1"}\nProjected impact: +${nextMove?.lift || 0} points → ${nextMove?.projected || dScore}\n\n${stepsDone}/${stepsTotal} roadmap steps completed.\n${daysSince} days since assessment.\n\n— Generated by RunPayway™ Command Center`;
              navigator.clipboard.writeText(text);
            };

            return (
              <div style={{ marginBottom: 24, animation: "fadeSlideIn 400ms ease-out" }}>
                <div style={{ padding: mobile ? "24px 20px" : "28px 32px", borderRadius: 16, backgroundColor: B.surface, border: `1px solid ${B.teal}15`, boxShadow: "0 1px 4px rgba(14,26,43,0.03)" }}>
                  {/* Header */}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: `${B.teal}08`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={B.teal} strokeWidth="2" strokeLinecap="round"><path d="M12 8v4l3 3m6-3a9 9 0 1 1-18 0 9 9 0 0 1 18 0z" /></svg>
                      </div>
                      <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.10em", color: B.teal }}>THIS WEEK</span>
                    </div>
                    <button onClick={copyBriefing} style={{ fontSize: 12, fontWeight: 600, color: B.muted, padding: "6px 12px", borderRadius: 6, border: `1px solid ${B.stone}`, backgroundColor: "transparent", cursor: "pointer", minHeight: 32, transition: "color 150ms" }}
                  ...
                  {/* Priority */}
                  <div style={{ fontSize: mobile ? 18 : 22, fontWeight: 600, color: B.navy, lineHeight: 1.3, marginBottom: 12 }}>
                    {stepsDone >= stepsTotal
                      ? "All roadmap steps complete. Consider a reassessment."
                      : nextMove
                        ? <>{nextStep ? `Step ${roadmap.indexOf(nextStep) + 1}: ` : ""}{nextMove.label}.</>
                        : "Start with your first structural move."
                    }
                  </div>

                  {nextMove && stepsDone < stepsTotal && (
                    <p style={{ fontSize: 15, color: B.muted, margin: "0 0 16px", lineHeight: 1.6 }}>
                      This is your highest-leverage conversation right now. It is projected to add <span style={{ fontWeight: 600, fontFamily: mono, color: B.teal }}>+{nextMove.lift}</span> points to your score.
                    </p>
                  )}

                  {/* Action + Since Last Visit */}
                  <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" as const }}>
                    {nextScript && stepsDone < stepsTotal && (
                      ...
                      }} style={{ fontSize: 14, fontWeight: 600, color: B.white, padding: "10px 20px", borderRadius: 10, backgroundColor: B.navy, border: "none", cursor: "pointer", minHeight: 44, transition: "background 150ms" }}
                        onMouseEnter={e => { e.currentTarget.style.backgroundColor = "#2a2248"; }}
                        onMouseLeave={e => { e.currentTarget.style.backgroundColor = B.navy; }}>
                        Open Script
                      </button>
                    )}
                    <div style={{ fontSize: 13, color: B.taupe, lineHeight: 1.5 }}>
                      {daysSince === 0 ? "Assessed today" : `${daysSince} day${daysSince === 1 ? "" : "s"} since assessment`}
                      {" · "}
                      <span style={{ fontWeight: 600, color: stepsDone > 0 ? B.teal : B.muted }}>{stepsDone}/{stepsTotal} steps done</span>
                      {progressLift > 0 && <> · Projected: <span style={{ fontWeight: 600, fontFamily: mono, color: B.teal }}>{projectedWithProgress} (+{progressLift})</span></>}
                    </div>
                  </div>
                </div>
              </div>
