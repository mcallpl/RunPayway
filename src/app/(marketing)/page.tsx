...
const INDUSTRIES = [
  { key: "consulting", slug: "consulting", name: "Consulting", constraint: "You are the product", desc: "Your income depends on billable hours, not repeating work. RunPayway measures what % is one client and how much requires continuous new selling.", prep: ["Know what % of your income comes from your single largest client", "Know whether your current work is contracted, retainer, or project-by-project", "Think about what your income looks like in month 2 if you sign nothing new today"], unlocks: [{ before: "You guessed whether income was stable enough to hire or expand.", now: "You know exactly how much is dependable before committing to a fixed cost." }, { before: "Diversify was the advice. No one told you which risk to fix first.", now: "The score ranks your constraints — concentration, recurrence, visibility — so you fix the right one." }] },
  { key: "real_estate", slug: "real-estate", name: "Real Estate", constraint: "Pipeline dependency", desc: "Your income depends on closed deals, not signed contracts. RunPayway measures what's contractually locked (closed) vs. assumed (pipeline), showing your true secured floor.", prep: ["Know how many deals are under signed contract vs. verbal pipeline right now", "Know roughly what % of last year's income came from your top 1–2 closings", "Think about whether you have any income that continues between transactions"], unlocks: [{ before: "You used recent closings and pipeline confidence to gauge financial readiness.", now: "You see how much income is contractually secured vs. assumed before any major commitment." }, { before: "Strong quarters masked how much income restarted from zero each cycle.", now: "You can quantify the gap and build a floor that doesn't require a closing to exist." }] },
  { key: "sales", slug: "sales-brokerage", name: "Sales / Brokerage", constraint: "Nothing carries forward", desc: "Commission resets each period with no automatic carry. RunPayway measures what % repeats vs. requires a new close, revealing your actual recurring income floor.", prep: ["Know what % of this quarter's earnings will carry forward automatically", "Know how many separate commission sources your income comes from", "Think about what your income is in a month where no deals close"], unlocks: [{ before: "Last quarter's performance was the only data point for forward planning.", now: "You know what % of next quarter's income carries forward vs. requires a new close." }, { before: "Diversification was a goal with no starting point.", now: "The score shows how many sources your income depends on — and what to add first." }] },
  { key: "creative", slug: "freelance-creative", name: "Freelance / Creative", constraint: "Every month starts at zero", desc: "Your income structure has no automatic carry without new projects. RunPayway measures whether you have a real floor or genuinely restart monthly, and how much recurring work could change that.", prep: ["Know how many clients are actively paying you right now", "Know whether any of your income repeats without you re-selling it", "Think about what your income would be if you signed nothing new for 30 days"], unlocks: [{ before: "Inconsistency was a feeling. You had no way to measure how fragile the structure was.", now: "You can see whether your income has a real floor or genuinely resets every 30 days." }, { before: "Retainers were a nice-to-have. No number attached to why they mattered.", now: "You know exactly how much your score moves when you convert a project client to recurring." }] },
  { key: "construction", slug: "construction-trades", name: "Construction / Trades", constraint: "The next job isn't signed yet", desc: "Your income depends on signed backlog, not assumed pipeline. RunPayway shows you how many months of secured work you have vs. handshake agreements, measuring your gap between current and safe expansion capacity.", prep: ["Know how many months of signed work you currently have in backlog", "Know what % of your jobs come from repeat vs. new customers", "Think about whether your income stops completely between projects"], unlocks: [{ before: "You hired or bought equipment based on current backlog and gut feel.", now: "You see how many months of income are signed vs. assumed before committing to fixed costs." }, { before: "The next job isn't signed yet was anxiety with no number.", now: "You quantify the gap between current backlog and safe expansion capacity." }] },
  { key: "media", slug: "media-entertainment", name: "Media / Entertainment", constraint: "Between projects, income is zero", desc: "Your income structure has no residual between engagements. RunPayway measures your income floor—how much holds without an active project—and quantifies the gap you need to fill.", prep: ["Know how many months your current projects or contracts run", "Know whether you have any income that continues between active engagements", "Think about what your income looks like in a month with no active project"], unlocks: [{ before: "You made commitments during strong project phases without accounting for the gaps.", now: "You see your income floor — how much holds without an active project — before deciding." }, { before: "Between-project income of zero felt unavoidable.", now: "You can measure the gap and build toward a floor that doesn't depend on being active." }] },
  { key: "insurance", slug: "insurance", name: "Insurance", constraint: "New business masks renewal erosion", desc: "Your income needs both renewals and new production to hold. RunPayway measures the exact split between renewal stability and growth dependency, showing whether your structure is actually strengthening.", prep: ["Know what % of your income is renewals vs. new production", "Know whether your renewal book is growing, flat, or declining", "Think about how much your income would drop if new sales stopped for 90 days"], unlocks: [{ before: "Strong production quarters felt like growth, even when renewal erosion was underneath.", now: "You see whether your income is actually strengthening or just masked by new volume." }, { before: "Protect your renewal book had no number attached.", now: "You know the exact split between renewal stability and new production dependency." }] },
  { key: "legal", slug: "legal-services", name: "Legal Services", constraint: "Concentration in top matters", desc: "Your top 2–3 matters likely carry most of your billings. RunPayway measures what % of income depends on your largest clients and what a departure actually costs you in dollar terms and stability score movement.", prep: ["Know how many active matters you currently have", "Know what % of your billings come from your top 2–3 matters", "Think about whether any of your income is on retainer vs. matter-by-matter"], unlocks: [{ before: "You knew a few matters carried most of your billings. You couldn't quantify the exposure.", now: "You see what % of income depends on your top 2–3 matters and exactly what a departure costs." }, { before: "Retainer clients were a diversification goal with no urgency.", now: "You know specifically how much your score improves when recurring work replaces matter-by-matter billing." }] },
  { key: "technology", slug: "technology", name: "Technology", constraint: "One employer, one system", desc: "Your compensation structure is concentrated in one role. RunPayway measures what % is variable vs. base, how much outside income exists, and what your true income floor is independent of your primary position.", prep: ["Know what % of your total compensation is variable — bonus, equity, or commissions", "Know how many income sources you have outside your primary employer", "Think about what your income looks like if your role was eliminated today"], unlocks: [{ before: "Your compensation felt stable because the system around it was stable.", now: "You see how much of your total income is one restructuring decision away from disappearing." }, { before: "Variable comp felt like upside. You didn't treat it as a stability risk.", now: "You know your true income floor before any major borrowing or investment decision." }] },
  { key: "finance", slug: "finance-banking", name: "Finance / Banking", constraint: "Base salary + variable split", desc: "Your income has a fixed component and a variable component that cycles. RunPayway measures the split and your true stable floor—what holds if variable comp doesn't materialize.", prep: ["Know what % of your annual compensation is variable vs. fixed base", "Know whether your base salary alone would cover your financial obligations", "Think about how tied your income is to market conditions or deal flow"], unlocks: [{ before: "You quoted total compensation when making financial decisions.", now: "You know your actual stable floor — what holds if the variable component doesn't come through." }, { before: "A strong cycle felt the same as a stable income.", now: "You can separate cyclical performance from structural reliability — and plan against the real number." }] },
  { key: "healthcare", slug: "healthcare", name: "Healthcare", constraint: "One institution, no alternatives", desc: "Your income depends on one employer with no outside sources. RunPayway measures how concentrated your income is in that one institution and what percentage would remain if your role changed.", prep: ["Know whether your income comes from one institution or multiple sources", "Know what % of your income could continue if your primary role changed", "Think about whether any of your income continues when you're not actively working"], unlocks: [{ before: "Steady institutional pay felt like stable income.", now: "You see how concentrated your income is in one employer — and what actually holds in a restructure." }, { before: "You had no way to quantify how exposed one-system dependency made you.", now: "You know your stability score and can plan alternatives before a disruption, not after." }] },
  { key: "retail", slug: "retail-ecommerce", name: "Retail / E-Commerce", constraint: "Revenue is not income", desc: "Sales look strong until you subtract costs. RunPayway separates revenue from actual stable income, measuring monthly variability and how much is recurring vs. requiring constant new acquisition.", prep: ["Know how much your monthly revenue varies from your best to your slowest month", "Know whether you rely on one core product or have multiple lines", "Think about what % of your revenue comes from returning vs. new customers"], unlocks: [{ before: "Revenue numbers drove all financial decisions.", now: "You can separate revenue from actual stable income and plan against what you keep, not what you make." }, { before: "Customer retention was a marketing metric, not a stability signal.", now: "You see how much of your income is recurring vs. requires constant new acquisition — and which to prioritize." }] },
  { key: "hospitality", slug: "hospitality", name: "Hospitality / Food Service", constraint: "Demand-dependent income", desc: "Your income depends on customer demand without contractual protection. RunPayway measures income variability month-to-month, quantifies contracted vs. walk-in revenue, and shows you what your real slow-season floor is.", prep: ["Know what your income looks like in your slowest month of the year", "Know how much of your revenue is tied to foot traffic vs. contracted events", "Think about how quickly your income would change if demand shifted"], unlocks: [{ before: "Slow seasons felt like bad luck. No way to plan against them structurally.", now: "You can quantify income variability and size cash reserves before the slow period, not during it." }, { before: "You knew foot traffic was unpredictable but had no way to structure around it.", now: "You see what % of income is contracted vs. dependent on daily demand — and what to build from there." }] },
  { key: "transportation", slug: "transportation-logistics", name: "Transportation / Logistics", constraint: "Utilization-dependent income", desc: "Your income depends on constant utilization. RunPayway measures client/route concentration, shows what your largest client represents as a % of income, and quantifies how much your stability structure can absorb.", prep: ["Know how many active clients or contracts you currently have", "Know what happens to your income if your largest route or client pauses", "Think about whether your income is consistent or heavily tied to utilization"], unlocks: [{ before: "You knew which clients mattered. You couldn't quantify how exposed that made you.", now: "You see exactly what your largest route or client represents — and what losing it does to your floor." }, { before: "Adding capacity felt like a volume call.", now: "You know whether your income structure is stable enough to support new fixed costs before committing." }] },
  { key: "manufacturing", slug: "manufacturing", name: "Manufacturing", constraint: "Buyer concentration risk", desc: "Your largest buyer likely represents a significant % of income. RunPayway measures buyer concentration, separates contracted vs. ongoing relationships, and shows how buyer loss impacts your stability vs. fixed costs.", prep: ["Know what % of your output goes to your single largest buyer", "Know whether your current contracts are signed agreements or ongoing relationships", "Think about how your fixed costs compare to your income in a slow month"], unlocks: [{ before: "Your largest buyer was an assumed constant.", now: "You can see what % of output and income depends on a single contract — and what renegotiation actually means." }, { before: "Diversify your buyer base was strategic advice with no urgency.", now: "The score quantifies concentration so you pursue diversification with a specific target, not a general goal." }] },
  { key: "education", slug: "education", name: "Education", constraint: "Institutional concentration", desc: "Your income depends on one institution. RunPayway measures how concentrated your income is in that source and what outside income exists, quantifying how much a second source would improve your stability.", prep: ["Know whether your income comes from one institution or multiple sources", "Know what % of your income could continue if your primary position ended", "Think about whether you have any income outside your primary teaching or instruction role"], unlocks: [{ before: "Income felt secure because the institution felt stable.", now: "You see how concentrated your income is in one source — and what position elimination actually does to your floor." }, { before: "Outside income felt like extra. You didn't account for what it did to your stability.", now: "You know how much a second source moves your score — and what the gap costs you in financial flexibility." }] },
  { key: "nonprofit", slug: "nonprofit", name: "Nonprofit / Public Sector", constraint: "Funding cycle dependency", desc: "Your income depends on funding cycles beyond your control. RunPayway measures what % of your income is grant-funded vs. budget-funded, when cycles end, and how much depends on renewal vs. permanent funding.", prep: ["Know whether your position is grant-funded, budget-funded, or both", "Know when your current funding cycle ends", "Think about whether your income could continue if the primary funding source changed"], unlocks: [{ before: "You assumed stability as long as the organization was funded.", now: "You see exactly when your funding cycle ends and how much income depends on that renewal." }, { before: "Grant-funded income felt stable. The risk was abstract.", now: "You can quantify how much of your stability score depends on a single funding source being renewed." }] },
  { key: "agriculture", slug: "agriculture", name: "Agriculture", constraint: "Uncontracted income exposure", desc: "Your income is exposed to weather and market price swings. RunPayway measures contracted vs. open market sales, quantifies year-to-year income variance, and shows what % depends on any single crop, product, or buyer.", prep: ["Know whether you have any contracted sales vs. open market sales", "Know how much last year's income varied from a typical year", "Think about what % of your income comes from one crop, product, or buyer"], unlocks: [{ before: "You planned against average years without knowing how structurally exposed you were.", now: "You see how much income is contractually protected vs. open to market price and yield variance." }, { before: "Diversification felt theoretical — no number to act on.", now: "The score identifies your primary constraint — one crop, one buyer, price exposure — so you fix the right thing first." }] },
  { key: "other", slug: "other", name: "Other", constraint: "Non-standard income", desc: "If your income doesn't fit a standard category, RunPayway still measures your structure using six dimensions: diversification, recurrence, contracts, concentration, continuity, and forward visibility.", prep: ["Know your primary income sources and what each contributes", "Note what % of your income is recurring, contracted, or guaranteed", "Think about how dependent your income is on your direct effort"], unlocks: [{ before: "Your unique income type had no measurement framework.", now: "You see your stability using the same six structural dimensions that apply to every income type." }, { before: "You couldn't compare your stability to anyone else because you were in a different category.", now: "Your score places you in the structural stability scale — regardless of industry." }] },
  { key: "energy", slug: "energy-utilities", name: "Energy / Utilities", constraint: "Price exposure", desc: "Your margin depends on market prices you don't control. RunPayway measures what % of your customers are on contracted rates vs. spot pricing, quantifies income variability month-to-month, and shows your true margin floor.", prep: ["Know whether your income is tied to commodity prices or contracted rates", "Know how much your income varied month-to-month last year", "Think about whether your customers are under long-term contracts or spot pricing"], unlocks: [{ before: "Revenue looked stable when commodity prices cooperated. The risk was invisible.", now: "You separate contracted income from commodity-tied income — and plan against the floor that actually holds." }, { before: "Lock in contracts was standard advice. No number attached.", now: "You see how much of your stability depends on spot pricing — and what converting one customer does to your score." }] },
];
          ...
          <div style={{ maxWidth: 680, margin: "0 auto", borderRadius: 24, backgroundColor: C.navy, boxShadow: "0 16px 56px rgba(14,26,43,0.18), 0 4px 16px rgba(14,26,43,0.08)", position: "relative" as const, overflow: "hidden" }}>
            ...
            <div style={{ padding: m ? "32px 24px" : "44px 48px" }}>
              ...
              <div style={{ padding: m ? "16px 18px" : "18px 24px", borderRadius: 14, backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", marginBottom: 28 }}>
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.10em", color: "rgba(244,241,234,0.30)", marginBottom: 16 }}>WHAT THIS UNLOCKS</div>
                <div style={{ display: "flex", flexDirection: "column" as const, gap: 0 }}>
                  {active.unlocks.map((item, i) => (
                    <div key={i} style={{ paddingBottom: i < active.unlocks.length - 1 ? 14 : 0, marginBottom: i < active.unlocks.length - 1 ? 14 : 0, borderBottom: i < active.unlocks.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                      <div style={{ display: "flex", gap: 8, marginBottom: 5, alignItems: "flex-start" }}>
                        <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", color: "rgba(244,241,234,0.28)", flexShrink: 0, paddingTop: 2, minWidth: 44 }}>BEFORE</span>
                        <span style={{ fontSize: m ? 13 : 14, color: "rgba(244,241,234,0.48)", lineHeight: 1.5 }}>{item.before}</span>
                      </div>
                      <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                        <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", color: C.teal, flexShrink: 0, paddingTop: 2, minWidth: 44 }}>NOW</span>
                        <span style={{ fontSize: m ? 13 : 14, color: "rgba(244,241,234,0.82)", lineHeight: 1.5, fontWeight: 500 }}>{item.now}</span>
                      </div>
                    </div>
                  ))}
                </div>
